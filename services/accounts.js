...
class AccountService {
    ...
    async getProfile(nexoId) {
        const [users] = await db.execute('SELECT nickname, nexo_id, avatar_seed, created_at FROM users WHERE nexo_id = ?', [nexoId]);
        return users.length > 0 ? users[0] : null;
    }

    // ACTUALIZAR PERFIL COMPLETO
    async updateProfile(nexoId, data) {
        const { nickname, avatar_seed, currentPassword, newPassword } = data;

        // 1. Cambio de apodo y avatar
        if (nickname) {
            await db.execute('UPDATE users SET nickname = ?, avatar_seed = ? WHERE nexo_id = ?', [nickname, avatar_seed, nexoId]);
        }
